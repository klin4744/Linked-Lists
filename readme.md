...
#### When should I use multiple pointers?

It's hard to tell when using multiple pointers would be useful. If your input is sorted, if you need to reverse something, or if you need to keep track of something for some reason, multiple pointers can be helpful, but not always.
...
In our first iteration, our max sum is -1, we look ahead to 2, since 2+-1 is greater than 1, we move our end pointer forwards
...
#### When should I use a shifting frame?

Shifting frame problems are pretty easy to detect. The words continuous, contiguous, and consecutive tend to be signs that you'll need to use a shifting frame. Additionally if you have to collect a "frame" or set of consecutive points, a shifting frame is probably the way to go.

### Utilizing Binary Search and Counting Sort

Many algorithms can be made more efficient with the use of either Binary Search or Counting Sort. These problems tend to be capable of being handled without either binary search or counting sort.
